...
if [[ -f "${SCRIPT_DIR}/train_once.py" ]]; then
    REPO_DIR="${SCRIPT_DIR}"
elif [[ -f "${SCRIPT_DIR}/../train_once.py" ]]; then
    REPO_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
else
    echo "ERROR: Could not locate train_once.py next to this script or in parent directory"
    exit 1
fi
LOG_DIR="${REPO_DIR}/logs"
TRAIN_SCRIPT="${REPO_DIR}/train_once.py"
...
# Keep TensorFlow thread usage aligned with requested CPUs.
